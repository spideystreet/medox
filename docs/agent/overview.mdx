...
L'agent Medox est implémenté avec **LangGraph** comme machine à états finie. Il suit le pattern **ReAct** (Reasoning + Acting) : le LLM alterne entre raisonnement et appels d'outils jusqu'à produire une réponse finale.
...
```python
from medox.agent.graph_agent import graph
